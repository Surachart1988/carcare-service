<template>
  <div class="component-wrapper">
    ...
    <Modal
      ...
      v-model="modalAddDetailData"
      :mask-closable="false"
      width="90%"
      @on-cancel="closeModalAddDetail()"
    >
      <p slot="header">
      ...
      >
        ...
        <FormItem
          ...
          flex-offset-gt-md="0"
        >
          <Input size="large" v-model="formItem.field16"></Input>
        </FormItem>
      </Form>
      <div slot="footer">
        <div layout="row" layout-align="center center">
          <Button
            ...
            >บันทึก</Button
          >
          <Button type="error" shape="circle" size="large" @click="closeModalAddDetail()"
            >ยกเลิก</Button
          >
        </div>
      </div>
    ...
    >
      <p slot="header">
        <span>สร้างใบสั่งซื้อสินค้า</span>
      </p>
...
export default {
  ...
  computed: {
    apiUrl() {
      return `api/v1/purchaseorder?pageType=null&pageNumber=${this.dataCurrentPage}&pageSize=${this.dataPageSize}&orderAsc=${this.dataSortBy.order}&orderBy=${this.dataSortBy.key}`;
    },
  },
  mounted() {
    this.getData();
  ...
  methods: {
    ...
    openModalAddDetail() {
      this.modalAddDetailData = true;
      this.modalAddData = false;
    },
    closeModalAddDetail() {
      this.modalAddDetailData = false;
      this.modalAddData = true;
    },
    editRowData(index) {
      this.$Modal.info({
