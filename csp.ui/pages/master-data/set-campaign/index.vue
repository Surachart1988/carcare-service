<template>
  <div class="component-wrapper">
    ...
    >
      ...
      <Form ref="inputForm" :model="formItem" :rules="ruleValidate" layout="row" layout-wrap>
        <FormItem prop="field1" :label-width="205" label="ชื่อแคมเปญ" flex-xs="100" flex-sm="100" flex="90">
      ...
      <div slot="footer">
        <div layout="row" layout-align="center center">
          <Button type="success" shape="circle" size="large" @click="handleSubmit('inputForm')">บันทึก</Button>
          <Button type="error" shape="circle" size="large" @click>ยกเลิก</Button>
        </div>
      </div>
    </Modal>
  </div>
...
export default {
  ...
  computed: {
    apiUrl() {
      return `api/v1/setting-campaign?pageType=null&pageNumber=${this.dataCurrentPage}&pageSize=${this.dataPageSize}&orderAsc=${this.dataSortBy.order}&orderBy=${this.dataSortBy.key}`
    }
  },
  mounted() {
    this.getData();
