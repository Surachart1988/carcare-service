<template>
  <div class="component-wrapper">
    ...
    <HeaderPage :userAvatar="false" :buttonAdd="true" :divider="false">กลุ่มสินค้า</HeaderPage>
    ...
    <section v-if="vPermisson">
      <DataTable
        ...
        page-type="productgroup"
        ref="dataTable"
        :mode-search="modeSearch"
      ></DataTable>

      <Modal v-show="modalAddData"
        v-model="resloveSlowInput" :mask-closable="false" width="60%" @on-cancel="closeModalAdd()">
        <p slot="header">
          <span>เพิ่มข้อมูลกลุ่มสินค้า</span>
        </p>
        <Form
          ref="formValidate"
          :model="formItem"
        ...
        >
          ...
          <FormItem prop="Code" label="รหัสกลุ่มสินค้า" flex="100" flex-gt-md="50">
            <Input size="large" v-model="formItem.Code" placeholder></Input>
          </FormItem>
          <FormItem prop="Name" label="กลุ่มสินค้า" flex="100" flex-gt-md="50">
...
export default {
  ...
  methods: {
    ...
    async editData(id) {
      this.itemID = id
      let apiWithQuery = `${'api/v1/MasterData/' + id + '?pageType=productgroup'}`
    ...
    async saveData() {
      ...
      let apiUrl = `api/v1/MasterData?pageType=productgroup`
      let apiEditMode = `${'api/v1/MasterData' + '/' + this.itemID + '?pageType=productgroup'}`
