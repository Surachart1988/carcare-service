<template>
  <div class="component-wrapper">
    ...
    <HeaderPage :userAvatar="false" :buttonAdd="true" :divider="false">ประเภทธุรกิจ</HeaderPage>
    ...
    <Modal v-show="modalAddData" v-model="resloveSlowInput" :mask-closable="false" width="60%" @on-cancel="closeModalAdd()">
      <p slot="header">
        <span>เพิ่มข้อมูลประเภทธุรกิจ</span>
      </p>
      <Form
          ref="formValidate"
          :model="formItem"
...
export default {
  ...
  methods: {
    ...
    async editData(id) {
      this.itemID = id
      let apiWithQuery = `${'api/v1/MasterData/' + id + '?pageType=bussinesstype'}`
    ...
    async saveData() {
      ...
      let apiUrl = `api/v1/MasterData?pageType=bussinesstype`
      let apiEditMode = `${'api/v1/MasterData' + '/' + this.itemID + '?pageType=bussinesstype'}`
