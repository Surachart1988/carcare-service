<template>
  <div class="component-wrapper">
    ...
    <HeaderPage :userAvatar="false" :buttonAdd="true" :divider="false">หน่วยสินค้า</HeaderPage>
    ...
    <section v-if="vPermisson">
      ...
      <Modal v-show="modalAddData" v-model="resloveSlowInput" :mask-closable="false" width="60%" @on-cancel="closeModalAdd()">
        <p slot="header">
          <span>เพิ่มข้อมูลหน่วยสินค้า</span>
        </p>
        <Form
          ref="formValidate"
          :model="formItem"
        ...
        >
          <div layout="row" flex="100" flex-lg="80" flex-gt-lg="80">
...
export default {
  ...
  methods: {
    ...
    async editData(id) {
      this.itemID = id
      let apiWithQuery = `${'api/v1/MasterData/' + id + '?pageType=productunit'}`
    ...
    async saveData() {
      ...
      let apiUrl = `api/v1/MasterData?pageType=productunit`
      let apiEditMode = `${'api/v1/MasterData' + '/' + this.itemID + '?pageType=productunit'}`
