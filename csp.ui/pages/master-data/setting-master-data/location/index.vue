<template>
  <div class="component-wrapper">
    ...
    <HeaderPage :userAvatar="false" :buttonAdd="true" :divider="false">ที่เก็บสินค้า</HeaderPage>
    ...
    <Modal v-model="modalAddData" :mask-closable="false" width="60%" @on-cancel="closeModalAdd()">
      <p slot="header">
        <span>เพิ่มข้อมูลที่เก็บสินค้า</span>
      </p>
      <Form
        ref="formValidate"
        :model="formItem"
...
export default {
  ...
  methods: {
    ...
    async editData(id) {
      this.itemID = id
      let apiWithQuery = `${'api/v1/MasterData/' + id + '?pageType=location'}`
    ...
    async saveData() {
      ...
      let apiUrl = `api/v1/MasterData?pageType=location`
      let apiEditMode = `${'api/v1/MasterData' + '/' + this.itemID + '?pageType=location'}`
