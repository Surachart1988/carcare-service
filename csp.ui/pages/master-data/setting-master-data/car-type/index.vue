<template>
  <div class="component-wrapper">
    ...
    <HeaderPage :userAvatar="false" :buttonAdd="true" :divider="false">ประเภทรถยนต์</HeaderPage>
    ...
    <section v-if="vPermisson">
      ...
      <Modal v-show="modalAddData" v-model="resloveSlowInput" :mask-closable="false" width="60%" @on-cancel="closeModalAdd()">
        <p slot="header">
          <span>เพิ่มข้อมูลประเภทรถยนต์</span>
        </p>
        <Form
          ref="formValidate"
          :model="formItem"
        ...
        >
          <div layout="row" flex="100" flex-gt-lg="80" layout-align="center center">
            <FormItem prop="Name" flex="100" :label="tableColumns1[1].title">
...
export default {
  ...
  methods: {
    ...
    async editData(id) {
      this.itemID = id
      let apiWithQuery = `${'api/v1/MasterData/' + id + '?pageType=cartype'}`
    ...
    async saveData() {
      ...
      let apiUrl = `api/v1/MasterData?pageType=cartype`
      let apiEditMode = `${'api/v1/MasterData' + '/' + this.itemID + '?pageType=cartype'}`
