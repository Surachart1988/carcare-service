<template>
  <div class="component-wrapper">
    ...
    <HeaderPage :userAvatar="false" :buttonAdd="true" :divider="false">รุ่นสินค้า</HeaderPage>
    ...
    <section v-if="vPermisson">
      ...
      <Modal v-show="modalAddData" v-model="resloveSlowInput" :mask-closable="false" width="60%" @on-cancel="closeModalAdd()">
        <p slot="header">
          <span>เพิ่มข้อมูลรุ่นสินค้า</span>
        </p>
        <Form
          ref="formValidate"
          :model="formItem"
        ...
        >
          <div layout="row" flex="100" flex-gt-lg="80">
            <FormItem prop="ProBrandID" :label="tableColumns1[1].title" flex="100" flex-gt-md="50">
              <Select size="large" v-model="formItem.ProBrandID">
                <Option v-for="(e, index) in getDataProBrand" :key="index" :value="e.Id">{{e.Name}}</Option>
              </Select>
            </FormItem>
...
export default {
  ...
  data() {
    return {
      ...
      ruleValidate: {
        ProBrandID: [{ type: 'number', required: true, message: 'กรุณากรอกข้อมูล', trigger: 'change' }],
      ...
      tableColumns1: [
        ...
        {
          title: 'ยี่ห้อสินค้า',
          key: 'ProBrandName', //มาจากตารางอื่น
          minWidth : 160,
          align: 'center',
          sortable: true
        },
  ...
  methods: {
    async productbrandFromAPI() {
      ...
      if (StatusCode == 200) {
        this.getDataProBrand = await Resource
        console.log('productbrandFromAPI()', this.getDataPosition)
      }
    ...
    async editData(id) {
      this.itemID = id
      let apiWithQuery = `${'api/v1/MasterData/' + id + '?pageType=productmodel'}`
    ...
    async saveData() {
      ...
      let apiUrl = `api/v1/MasterData?pageType=productmodel`
      let apiEditMode = `${'api/v1/MasterData' + '/' + this.itemID + '?pageType=productmodel'}`
