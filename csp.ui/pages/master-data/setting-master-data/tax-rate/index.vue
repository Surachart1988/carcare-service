<template>
  <div class="component-wrapper">
    ...
    <HeaderPage :userAvatar="false" :buttonAdd="true" :divider="false">อัตราภาษีซื้อ/ขาย</HeaderPage>
    ...
    <section v-if="vPermisson">
      ...
      <Modal v-show="modalAddData" v-model="resloveSlowInput" :mask-closable="false" width="60%" @on-cancel="closeModalAdd()">
        <p slot="header">
          <span>เพิ่มข้อมูลอัตราภาษีซื้อ/ขาย</span>
        </p>
        <Form
          ref="formValidate"
          :model="formItem"
        ...
        >
          <div layout="row" layout-wrap flex="100" flex-gt-lg="80">
            <FormItem prop="Name" label="ชื่ออัตราภาษี" flex="100" flex-gt-md="50">
              <Input size="large" v-model="formItem.Name" placeholder></Input>
            </FormItem>
            <FormItem prop="Cost" label="อัตราภาษี" flex="100" flex-gt-md="50">
              <InputNumber
                size="large"
...
export default {
  ...
  data() {
    return {
      ...
      ruleValidate: {
        ...
        Cost: [{ type: 'number', required: true, message: 'กรุณากรอกข้อมูล', trigger: 'blur' }]
      },
      tableColumns1: [
        {
          title: 'ลำดับ',
  ...
  methods: {
    ...
    async editData(id) {
      this.itemID = id
      let apiWithQuery = `${'api/v1/MasterData/' + id + '?pageType=inputtaxrate'}`
      ...
      if (StatusCode == 200) {
        ...
        ;(this.formItem.Cost = Resource.Cost), (this.formItem.Name = Resource.Name)
    ...
    async saveData() {
      ...
      let apiUrl = `api/v1/MasterData?pageType=inputtaxrate`
      let apiEditMode = `${'api/v1/MasterData' + '/' + this.itemID + '?pageType=inputtaxrate'}`
