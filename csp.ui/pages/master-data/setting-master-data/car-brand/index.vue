  ...
  <div class="component-wrapper">
    ...
    <section v-if="vPermisson">
      ...
      <Modal v-show="modalAddData" v-model="resloveSlowInput" :mask-closable="false" width="60%" @on-cancel="closeModalAdd()">
        <p slot="header">
          <span>เพิ่มข้อมูลยี่ห้อรถยนต์</span>
        </p>
        <Form
          ref="formValidate"
          :model="formItem"
        ...
        >
          <div layout="row" layout-wrap flex="100" flex-gt-lg="80">
            <FormItem prop="CarTypeId" flex="100" flex-gt-md="50" :label="tableColumns1[1].title">
              <Select size="large" v-model="formItem.CarTypeId">
                <Option v-for="(e, index) in getDataCarType" :key="index" :value="e.Id">{{e.Name}}</Option>
              </Select>
            </FormItem>
            <FormItem prop="Name" flex="100" flex-gt-md="50" :label="tableColumns1[2].title">
...
export default {
  ...
  data() {
    return {
      ...
      getDataCarType: [],
      ruleValidate: {
        CarTypeId: [{ type: 'number', required: true, message: 'กรุณากรอกข้อมูล', trigger: 'change' }],
      ...
      tableColumns1: [
        ...
        {
          title: 'ประเภทรถยนต์',
          key: 'CarTypeName', //มาจากตารางอื่น
          minWidth : 160,
          align: 'center',
          sortable: true
        },
  ...
  methods: {
    async cartypeFromAPI() {
      console.log('cartypeFromAPI()')

      let apiWithQuery = 'api/v1/MasterData/car/type'
      ...
      console.log('cartypeFromAPI()', res)

      let { StatusCode, Resource } = await res

      if (StatusCode == 200) {
        this.getDataCarType = await Resource
        console.log('cartypeFromAPI()', this.getDataPosition)
      }

      if (res == undefined) {
        await this.reToken()
        await this.cartypeFromAPI()
      }
    },
    ...
    async editData(id) {
      this.itemID = id
      let apiWithQuery = `${'api/v1/MasterData/' + id + '?pageType=carbrand'}`
      ...
      if (StatusCode == 200) {
        ...
        ;(this.formItem.CarTypeId = Resource.CarTypeId), (this.formItem.Name = Resource.Name)
    ...
    async saveData() {
      ...
      let apiUrl = `api/v1/MasterData?pageType=carbrand`
      let apiEditMode = `${'api/v1/MasterData' + '/' + this.itemID + '?pageType=carbrand'}`
      ...
      if (this.modeEdit == false) {
        let res = await this.$axios
          .$post(
            `${apiUrl}`,
            {
              CarTypeId: this.formItem.CarTypeId,
