<template>
  <div class="component-wrapper">
    ...
    <Form
      ...
      layout-wrap
    >
      <FormItem label="ค้นหา จากวันที่" flex-xs="100" flex-sm="100" flex-md="100" flex-gt-md="50">
        <DatePicker size="large" type="date" placeholder="เลือกวันที่" v-model="formItem.field1"></DatePicker>
      </FormItem>
      <FormItem label="ถึงวันที่" flex-xs="100" flex-sm="100" flex-md="100" flex-gt-md="50">
        <DatePicker size="large" type="date" placeholder="เลือกวันที่" v-model="formItem.field2"></DatePicker>
      </FormItem>
    </Form>
    ...
    >
      <template slot-scope="{ row, index }" slot="link1">
        <nuxt-link to="#">เงื่อนไข</nuxt-link>
      </template>
      <template slot-scope="{ row, index }" slot="link2">
        <nuxt-link to="#">
      ...
      <template slot-scope="{ row, index }" slot="action">
        <div layout="row" layout-align="center center">
          ...
          <Tooltip placement="top" content="ลบข้อมูล" layout="row" layout-align="center center">
            <svg
              class="btn-action"
    ...
    >
      ...
      >
        ...
        <FormItem
          ...
          flex-offset-gt-md="0"
        >
          <DatePicker size="large" type="date" placeholder="เลือกวันที่" v-model="formItem.field3"></DatePicker>
        </FormItem>
        <FormItem
          prop="field4"
          flex-xs="100"
        ...
        >
          <label class="ivu-form-item-label">สถานะการใช้งาน</label>
          <i-switch size="large" v-model="formItem.field4">
            <span slot="open">อนุญาต</span>
            <span slot="close">ไม่อนุญาต</span>
          </i-switch>
        </FormItem>
        <Divider class="main-divider" />
        <FormItem label="รายละเอียด" prop="field5" flex="100">
          <Input
            size="large"
...
export default {
  ...
  data() {
    return {
      ruleValidate: {
        ...
        field2: [{ required: true, type: 'date', message: 'กรุณากรอกข้อมูล', trigger: 'change' }],
        field3: [{ required: true, type: 'date', message: 'กรุณากรอกข้อมูล', trigger: 'change' }],
  ...
  computed: {
    apiUrl() {
      return `api/v1/setting-news?pageType=null&pageNumber=${this.dataCurrentPage}&pageSize=${this.dataPageSize}&orderAsc=${this.dataSortBy.order}&orderBy=${this.dataSortBy.key}`
    }
  },
  mounted() {
    this.getData()
