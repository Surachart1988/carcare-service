<template>
  <div class="component-wrapper">
    ...
    <Modal v-model="modalAddData" :mask-closable="false" width="90%" @on-cancel="closeModalAdd()">
      <p slot="header">
        <span>เพิ่มข้อมูลวัสดุสิ้นเปลือง/อุปกรณ์สำนักงาน</span>
      </p>
      <Form
        ref="inputForm"
        :model="formItem"
...
export default {
  ...
  computed: {
    apiUrl() {
      return `api/v1/supplies-registration?pageType=null&pageNumber=${this.dataCurrentPage}&pageSize=${this.dataPageSize}&orderAsc=${this.dataSortBy.order}&orderBy=${this.dataSortBy.key}`;
    },
  },
  mounted() {
    this.getData();
